...
export default class ShopScene extends Phaser.Scene {
    ...
    create(): void {
        ...
        const coinBalanceText = this.add.text(10, 10, `Coins: ${this.gameManager.coins}`, { fontSize: '24px', color: '#ffd700', fontStyle: 'bold' }).setScrollFactor(0);
        ...
        this.events.on('wake', () => {
            coinBalanceText.setText(`Coins: ${this.gameManager.coins}`);
        });
    }
    ...
    private purchaseWeapon(weapon, weaponText): void {
        if (this.gameManager.coins >= weapon.cost) {
            this.gameManager.coins -= weapon.cost;
            weaponText.setText(`${weapon.name} - Purchased`).setColor('#00ff00');
            // Adjust the call to match the new signature of setCurrentWeapon
            this.gameManager.setCurrentWeapon(weapon.image, weapon.name);
        } else {
